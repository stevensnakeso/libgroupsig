...
#ifndef _SCSl25_MGR_KEY_H
#define _SCSL25_MGR_KEY_H
...
/**
 * @def SCSL25_MGR_KEY_BEGIN_MSG
 * @brief Begin string to prepend to headers of files containing SCSL25 group keys
 */
#define SCSL25_MGR_KEY_BEGIN_MSG "BEGIN SCSL25 MANAGERKEY"

/**
 * @def SCSL25_MGR_KEY_END_MSG
 * @brief End string to prepend to headers of files containing SCSL25 group keys
 */
#define SCSL25_MGR_KEY_END_MSG "END SCSL25 MANAGERKEY"

/**
 * @struct scsl25_mgr_key_t
 * @brief SCSL25 Manager key.
 * 
 * The secret key for the issuing authority.
 */
typedef struct {
  pbcext_element_Fr_t *isk; /**< Issuer secret key. */
  pbcext_element_G1_t *p;   /**< 模数 n 的因数 p。 */
  pbcext_element_G1_t *q;   /**< 模数 n 的因数 q。 */
} scsl25_mgr_key_t;

/** 
 * @fn groupsig_key_t* scsl25_mgr_key_init()
 * @brief Creates a new SCSL25 manager key
 *
...
/** 
 * @fn int scsl25_mgr_key_free(groupsig_key_t *key)
...
/** 
 * @fn int scsl25_mgr_key_copy(groupsig_key_t *dst, groupsig_key_t *src)
...
/** 
 * @fn int scsl25_mgr_key_get_size_in_format(groupsig_key_t *key)
...
/** 
 * @fn int scsl25_mgr_key_export(byte_t **bytes, uint32_t *size, groupsig_key_t *key)
 * @brief Writes a bytearray representation of the given manager key to an array
 *  with format:
 *
 *  | SCSL25_CODE | KEYTYPE | size_isk | isk |
 *
...
/** 
 * @fn groupsig_key_t* scsl25_mgr_key_import(byte_t *source, uint32_t size)
 * @brief Imports a SCSL25 manager key from the specified source.
 *
...
/** 
 * @fn char* scsl25_mgr_key_to_string(mgr_key_t *key)
...
/**
 * @var scsl25_mgr_key_handle
 * @brief Set of functions for SCSL25 manager keys management.
 */
static const mgr_key_handle_t scsl25_mgr_key_handle = {
  .code = GROUPSIG_SCSL25_CODE, /**< The scheme code. */
  .init = &scsl25_mgr_key_init, /**< Initializes manager keys. */
  .free = &scsl25_mgr_key_free, /**< Frees manager keys. */
  .copy = &scsl25_mgr_key_copy, /**< Copies manager keys. */
  .gexport = &scsl25_mgr_key_export, /**< Exports manager keys. */
  .gimport = &scsl25_mgr_key_import, /**< Imports manager keys. */
  .to_string = &scsl25_mgr_key_to_string, /**< Converts manager keys to printable strings. */
  .get_size = &scsl25_mgr_key_get_size /**< Gets the size of the key, in bytes. */
};
