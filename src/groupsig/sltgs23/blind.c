...
int sltgs23_trace_blind(identity_t *nym, groupsig_blindsig_t *bsig, groupsig_key_t **bldkey,
  ...
  sltgs23_grp_key_t *sltgs23_grpkey;
  sltgs23_bld_key_t *sltgs23_bldkey;
  pbcext_element_G1_t *id;
  int rc;
  
  if(!bsig || bsig->scheme != GROUPSIG_SLTGS23_CODE ||
     !sig || sig->scheme != GROUPSIG_SLTGS23_CODE ||
     !grpkey || grpkey->scheme != GROUPSIG_SLTGS23_CODE ||
     !nym || nym->scheme != GROUPSIG_SLTGS23_CODE
    ) {
    LOG_EINVAL(&logger, __FILE__, "sltgs23_blind", __LINE__, LOGERROR);
    return IERROR;
  }
  ...
  sltgs23_grpkey = (sltgs23_grp_key_t *) grpkey->key;
  sltgs23_sig = (sltgs23_blindsig_t *) sig->sig;
  sltgs23_bsig = (sltgs23_blindsig_t *) bsig->sig;

  pbcext_element_G1_free(sltgs23_bsig->nym1); sltgs23_bsig->nym1 = NULL;
  pbcext_element_G1_free(sltgs23_bsig->nym2); sltgs23_bsig->nym2 = NULL;
  pbcext_element_G1_free(sltgs23_bsig->nym3); sltgs23_bsig->nym3 = NULL;
  pbcext_element_G1_free(sltgs23_bsig->c1); sltgs23_bsig->c1 = NULL;
  pbcext_element_G1_free(sltgs23_bsig->c2); sltgs23_bsig->c2 = NULL;
  ...
  if(!(sltgs23_sig->nym3 = pbcext_element_G1_init()))
    GOTOENDRC(IERROR, sltgs23_blind);
  if(!(sltgs23_sig->c1 = pbcext_element_G1_init())) //not used
    GOTOENDRC(IERROR, sltgs23_blind);
  if(!(sltgs23_sig->c2 = pbcext_element_G1_init())) //not used
    GOTOENDRC(IERROR, sltgs23_blind);
  ...
  if(pbcext_element_G1_mul(sltgs23_sig->nym1, sltgs23_grpkey->g, alpha) == IERROR)
    GOTOENDRC(IERROR, sltgs23_blind);
  if(pbcext_element_G1_mul(aux, sltgs23_grpkey->cpk, alpha) == IERROR)
    GOTOENDRC(IERROR, sltgs23_blind);
  if(pbcext_element_G1_add(sltgs23_sig->nym2, aux, id) == IERROR)
    GOTOENDRC(IERROR, sltgs23_blind);
  ...
  if(pbcext_element_G1_add(sltgs23_bsig->nym1, sltgs23_sig->nym1, aux) == IERROR)
    GOTOENDRC(IERROR, sltgs23_blind);
  if(pbcext_element_G1_mul(sltgs23_bsig->nym2, sltgs23_grpkey->g, gamma) == IERROR)
    GOTOENDRC(IERROR, sltgs23_blind);
  if(pbcext_element_G1_mul(aux, sltgs23_grpkey->cpk, beta) == IERROR)
    GOTOENDRC(IERROR, sltgs23_blind);
  if(pbcext_element_G1_add(sltgs23_bsig->nym3, sltgs23_sig->nym2, aux) == IERROR)
