...
int sltgs23_convert(groupsig_blindsig_t **csigs,
  ...
  if(!(r = pbcext_element_Fr_init())) GOTOENDRC(IERROR, sltgs23_convert); //r_id
  if(!(r1 = pbcext_element_Fr_init())) GOTOENDRC(IERROR, sltgs23_convert); //r_1.id
  if(!(r2 = pbcext_element_Fr_init())) GOTOENDRC(IERROR, sltgs23_convert); //r_2.id
  ...
  // if(msg!=NULL)
  // {
  //   if(message_free(msg) == IERROR) GOTOENDRC(IERROR, sltgs23_convert);
  //   msg = NULL;
  //   msg = message_init();
  // }
...
int sltgs23_trace_convert(groupsig_blindsig_t **csigs,
		 groupsig_blindsig_t **bsigs, uint32_t n_bsigs,
		 groupsig_key_t *grpkey, groupsig_key_t *mgrkey,
		 groupsig_key_t *bldkey, message_t *msg) {

  pbcext_element_Fr_t *r, *r1, *r2, *neg_csk, *r_id_inv;
  pbcext_element_G1_t *aux, *cnym1p, *cnym2p;
  sltgs23_blindsig_t *sltgs23_csig;
  sltgs23_blindsig_t *sltgs23_bsig;
  sltgs23_grp_key_t *sltgs23_grpkey;
  ...
  if(!(r = pbcext_element_Fr_init())) GOTOENDRC(IERROR, sltgs23_convert); //r_id
  if(!(r_id_inv = pbcext_element_Fr_init())) GOTOENDRC(IERROR, sltgs23_convert); //r_id^-1
  // msg convert to id 
  ...
  for(i=0; i<n_bsigs; i++) {
    ...
    /* Decrypt nym and raise to r */
    if(pbcext_element_G1_mul(cnym1p, sltgs23_bsig->nym2, r_id_inv) == IERROR)
      GOTOENDRC(IERROR, sltgs23_convert);
    if(pbcext_element_G1_mul(aux, sltgs23_bsig->nym1, neg_csk) == IERROR)
      GOTOENDRC(IERROR, sltgs23_convert);
    if(pbcext_element_G1_add(aux, aux, sltgs23_bsig->nym3) == IERROR)
      GOTOENDRC(IERROR, sltgs23_convert);
    if(pbcext_element_G1_mul(cnym2p, aux, r_id_inv) == IERROR)
      GOTOENDRC(IERROR, sltgs23_convert);
    
    /* Re-randomize nym */
    if(!(sltgs23_csig->nym1 = pbcext_element_G1_init()))
      GOTOENDRC(IERROR, sltgs23_convert);
    if(pbcext_element_G1_set(sltgs23_csig->nym1, cnym1p) == IERROR)
      GOTOENDRC(IERROR, sltgs23_convert);

    if(!(sltgs23_csig->nym2 = pbcext_element_G1_init()))
      GOTOENDRC(IERROR, sltgs23_convert);
    if(pbcext_element_G1_set(sltgs23_csig->nym2, cnym2p) == IERROR)
      GOTOENDRC(IERROR, sltgs23_convert);
 ...
 sltgs23_convert_end:
  ...
  pbcext_element_G1_free(cnym2p); cnym2p = NULL;
  pbcext_element_G1_free(aux); aux = NULL;
  pbcext_element_Fr_free(r_id_inv); r_id_inv = NULL;
