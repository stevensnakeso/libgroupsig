 ...
 */
typedef struct {
  pbcext_element_G1_t *A; /**< A = (H*h2^s*g1)^(1/isk+x) */  //A
  pbcext_element_Fr_t *x; /**< Randomly picked by the Issuer. */ //e
  pbcext_element_Fr_t *y; /**< Randomly picked by the Member. */ //y
  pbcext_element_Fr_t *s; /**< Randomly picked by the Issuer. */ //s
  /* Precomputations */
  pbcext_element_G1_t *H; /**< Member's "public key". H = h1^y. */ //C
  pbcext_element_G1_t *h2s; /**< Used in signatures. h2s = h2^s. */ //g2^s
} sltgs23_mem_key_t;
...
/** 
 * @fn int sltgs23_mem_key_get_size_in_format(groupsig_key_t *key)
 ...
 *  with format:
 *
 *  | SLTGS23_CODE | KEYTYPE | size_A | A | size_x | x | 
 ...
 *
 * Imports a SLTGS23 member key from the specified source, of the specified format.
 * 
...
static const mem_key_handle_t sltgs23_mem_key_handle = {
  ...
  .get_size = &sltgs23_mem_key_get_size, /**< Gets the size of the key as an array of bytes. */
  .gexport = &sltgs23_mem_key_export, /**< Exports member keys. */
  .gimport = &sltgs23_mem_key_import, /**< Imports member keys. */
  .to_string = &sltgs23_mem_key_to_string, /**< Converts member keys to printable strings. */
};
