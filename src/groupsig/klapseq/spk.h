...
#ifndef _KLAPSEQ_SPK_H
#define _KLAPSEQ_SPK_H
...
extern "C" {
  ...
  typedef struct _klapseq_spk0_t {
  ...
  typedef struct _klapseq_spk1_t {
  ...
  /**
   * @fn int klapseq_spk0_sign(spk_rep_t *pi,
   *                          void *y[], uint16_t ny,
   *	                      void *g[], uint16_t ng,
   *		              void *x[], uint16_t nx,
  ...
  /**
   * @fn klapseq_spk1_t* klapseq_spk1_init();
   * Initializes a data structures for SPKs used in open proofs in KLAPSEQ.
   *
  ...
  /**
   * @fn int klapseq_spk1_free(klapseq_spk1_t *pi);
  ...
  /**
   * @fn int klapseq_spk1_get_size(klapseq_spk1_t *pi);
   * Returns the memory needed to store, in an array of bytes, a data structures
   * for SPKs used in open proofs in KLAPSEQ.
   *
   * NOTE: This function will be removed with issue23.
   *
  ...
  /**
   * @fn int klapseq_spk1_export(byte_t **bytes, uint64_t *len, klapseq_spk1_t *pi);
   ...
   * @return The number of bytes, or -1 if error.
   */  
  int klapseq_spk1_export(byte_t **bytes, uint64_t *len, klapseq_spk1_t *pi);
  
  /**
   * @fn klapseq_spk1_t* klapseq_spk1_import(byte_t *bytes, uint64_t *len);
   * Imports a KLAPSEQ SPK of opening from the given array of bytes.
   *
   * NOTE: This function will be removed with issue23.
   *
  ...
  /**
   * @fn int klapseq_spk1_sign(klapseq_spk1_t *pi,
   ...
   *		              byte_t *msg,
   *		              uint32_t size);
   * Computes an SPK proving opening correctness in KLAPSEQ.
   *
   * NOTE: This function will be removed with issue23.
   *
   * @param[in,out] pi An initialized SPK structure for KLAPSEQ openings. 
  ...
  /**
   * @fn int klapseq_spk1_verify(uint8_t *ok,
   *                            klapseq_spk1_t *pi,
   *                            pbcext_element_G1_t *g1,
   *                            pbcext_element_G1_t *g2,
   *			        pbcext_element_GT_t *e1,
   ...
   *			        byte_t *msg,
   *			        uint32_t size);
   * Verifies an SPK proving opening correctness in KLAPSEQ.
   *
   * NOTE: This function will be removed with issue23.
   *
