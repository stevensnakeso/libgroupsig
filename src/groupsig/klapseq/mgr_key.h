...
#ifndef _KLAPSEQ_MGR_KEY_H
#define _KLAPSEQ_MGR_KEY_H
...
/**
 * @def KLAPSEQ_MGR_KEY_BEGIN_MSG
 * @brief Begin string to prepend to headers of files containing KLAPSEQ group keys
 */
#define KLAPSEQ_MGR_KEY_BEGIN_MSG "BEGIN KLAPSEQ MANAGERKEY"

/**
 * @def KLAPSEQ_MGR_KEY_END_MSG
 * @brief End string to prepend to headers of files containing KLAPSEQ group keys
 */
#define KLAPSEQ_MGR_KEY_END_MSG "END KLAPSEQ MANAGERKEY"

/**
 * @struct klapseq_mgr_key_t
 * @brief KLAPSEQ manager key. 
 */
typedef struct {
...
/** 
 * @fn groupsig_key_t* klapseq_mgr_key_init()
 * @brief Creates a new KLAPSEQ manager key
 *
...
/** 
 * @fn int klapseq_mgr_key_free(groupsig_key_t *key)
...
/** 
 * @fn int klapseq_mgr_key_copy(groupsig_key_t *dst, groupsig_key_t *src)
...
/** 
 * @fn int klapseq_mgr_key_get_size(groupsig_key_t *key)
...
/**
 * @fn int klapseq_mgr_key_export(byte_t **bytes, uint32_t *size, groupsig_key_t *key)
 * @brief Writes a bytearray representation of the given key, with format:
 *
 *  | KLAPSEQ_CODE | KEYTYPE | size_x | x | size_y | y | size_z0 | z0 | size_z1 | z1 |
 *
 ...
 * @return IOK or IERROR
 */
int klapseq_mgr_key_export(byte_t **bytes, uint32_t *size, groupsig_key_t *key);

/** 
 * @fn groupsig_key_t* klapseq_mgr_key_import(byte_t *source, uint32_t size)
 * @brief Imports a manager key.
 *
 * Imports a KLAPSEQ manager key from the specified array of bytes.
 * 
...
/** 
 * @fn char* klapseq_mgr_key_to_string(mgr_key_t *key)
...
/**
 * @var klapseq_mgr_key_handle
 * @brief Set of functions for KLAPSEQ manager keys management.
 */
static const mgr_key_handle_t klapseq_mgr_key_handle = {
  .code = GROUPSIG_KLAPSEQ_CODE, /**< The scheme code. */
  .init = &klapseq_mgr_key_init, /**< Initializes manager keys. */
  .free = &klapseq_mgr_key_free, /**< Frees manager keys. */
  .copy = &klapseq_mgr_key_copy, /**< Copies manager keys. */
  .gexport = &klapseq_mgr_key_export, /**< Exports manager keys. */
  .gimport = &klapseq_mgr_key_import, /**< Imports manager keys. */
  .to_string = &klapseq_mgr_key_to_string, /**< Converts manager keys to printable strings. */
  .get_size = &klapseq_mgr_key_get_size,
};
