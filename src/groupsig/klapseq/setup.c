...
int klapseq_setup(groupsig_key_t *grpkey,
  ...
  klapseq_grp_key_t *gkey;
  klapseq_mgr_key_t *mkey;
  pbcext_element_Fr_t *inv;
  int rc;
  uint8_t call;

  if(!grpkey || grpkey->scheme != GROUPSIG_KLAPSEQ_CODE ||
     !mgrkey || mgrkey->scheme != GROUPSIG_KLAPSEQ_CODE ||
     !gml) {
    LOG_EINVAL(&logger, __FILE__, "klapseq_setup", __LINE__, LOGERROR);
    return IERROR;
  }
  ...
  if (!gkey->g) {
    ...
    if(!(mkey->x = pbcext_element_Fr_init())) GOTOENDRC(IERROR, klapseq_setup);
    if(pbcext_element_Fr_random(mkey->x) == IERROR)
      GOTOENDRC(IERROR, klapseq_setup);

    if(!(mkey->y = pbcext_element_Fr_init())) GOTOENDRC(IERROR, klapseq_setup);
    ...
    if(!(gkey->g = pbcext_element_G1_init())) GOTOENDRC(IERROR, klapseq_setup);
    if(pbcext_element_G1_random(gkey->g) == IERROR)
      GOTOENDRC(IERROR, klapseq_setup);
    
    if(!(gkey->gg = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_setup);
    if(pbcext_element_G2_random(gkey->gg) == IERROR)
      GOTOENDRC(IERROR, klapseq_setup);
    
    /* Partially fill the group key with the Issuer's public key */
    if(!(gkey->XX = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_setup);
    if(pbcext_element_G2_mul(gkey->XX, gkey->gg, mkey->x) == IERROR)
      GOTOENDRC(IERROR, klapseq_setup);

    if(!(gkey->YY = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_setup);
   ...
   */
    ...
    if(!(mkey->z0 = pbcext_element_Fr_init())) GOTOENDRC(IERROR, klapseq_setup);
    if(pbcext_element_Fr_random(mkey->z0) == IERROR)
      GOTOENDRC(IERROR, klapseq_setup);

    if(!(mkey->z1 = pbcext_element_Fr_init())) GOTOENDRC(IERROR, klapseq_setup);
    if(pbcext_element_Fr_random(mkey->z1) == IERROR)
      GOTOENDRC(IERROR, klapseq_setup);       

    /* Finalize the group key with the Opener's public key */
    if(!(gkey->ZZ0 = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_setup);
    if(pbcext_element_G2_mul(gkey->ZZ0, gkey->gg, mkey->z0) == IERROR)
      GOTOENDRC(IERROR, klapseq_setup);

    if(!(gkey->ZZ1 = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_setup);
