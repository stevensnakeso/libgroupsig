...
int klapseq_spk0_sign(spk_rep_t *pi,
		     void *y[], uint16_t ny,
		     void *g[], uint16_t ng,
  ...
  if (!pi || !y || !g || !x || !i || !prods || !msg ||
    ...
    LOG_EINVAL(&logger, __FILE__, "klapseq_spk0_sign", __LINE__, LOGERROR);
    return IERROR;   
  }
  ...
  /* All loops in this function can probably be unified and make all 
     more efficient... */
  for(j=0; j<nx; j++) {
    if (!(r[j] = pbcext_element_Fr_init())) GOTOENDRC(IERROR, klapseq_spk0_sign);
    if (pbcext_element_Fr_random(r[j]) == IERROR)
      GOTOENDRC(IERROR, klapseq_spk0_sign);
  }
  ...
  if (!(gr[0] = pbcext_element_G1_init())) GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (pbcext_element_G1_mul(gr[0], g[i[0][1]], r[i[0][0]]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (!(gr[1] = pbcext_element_G1_init())) GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (pbcext_element_G1_mul(gr[1], g[i[1][1]], r[i[1][0]]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (!(gr[2] = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (pbcext_element_G2_mul(gr[2], g[i[2][1]], r[i[2][0]]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (!(gr[3] = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (pbcext_element_G2_mul(gr[3], g[i[3][1]], r[i[3][0]]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (!(gr[4] = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (pbcext_element_G2_mul(gr[4], g[i[4][1]], r[i[4][0]]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (!(gr[5] = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (pbcext_element_G2_mul(gr[5], g[i[5][1]], r[i[5][0]]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (!(gr[6] = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (pbcext_element_G2_mul(gr[6], g[i[6][1]], r[i[6][0]]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if (!(gr[7] = pbcext_element_G2_init())) GOTOENDRC(IERROR, klapseq_spk0_sign);
  ...
  */
  ...
  /* Push the message */
  if(!(hc = hash_init(HASH_BLAKE2))) GOTOENDRC(IERROR, klapseq_spk0_sign);
  if(hash_update(hc, msg, size) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_sign);
  
  /* Push the y values. Again, manually -- no loops */
  by = NULL;  
  if(pbcext_element_G1_to_bytes(&by, &len, y[0]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if(hash_update(hc, by, len) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_sign);
  mem_free(by); by = NULL;
  ...
  if(pbcext_element_G1_to_bytes(&bg, &len, g[0]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if(hash_update(hc, bg, len) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_sign);
  mem_free(bg); bg = NULL;
  ...
  for(j=0; j<ni; j++) {
    ...
    bi[2] = i[j][1] & 0xFF;
    bi[3] = (i[j][1] & 0xFF00) >> 8;
    if(hash_update(hc, bi, 4) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_sign);
  }
  ...
  if(pbcext_element_G1_to_bytes(&bprod, &len, prod[0]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_sign);
  if(hash_update(hc, bprod, len) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_sign);
  ...
  if(hash_finalize(hc) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_sign);
...
int klapseq_spk0_verify(uint8_t *ok,
		       void *y[], uint16_t ny,
		       void *g[], uint16_t ng,
		       uint16_t i[][2], uint16_t ni,
		       uint16_t *prods,
  ...
  if (!ok || !y || !g || !i || !prods || !pi || !msg ||
    ...
    LOG_EINVAL(&logger, __FILE__, "klapseq_spk0_verify", __LINE__, LOGERROR);
    return IERROR;   
  }
  ...
  if (!(gs1 = pbcext_element_G1_init()))
    GOTOENDRC(IERROR, klapseq_spk0_verify);
  ...
  */
  ...
  /* Push the message */
  if(!(hc = hash_init(HASH_BLAKE2))) GOTOENDRC(IERROR, klapseq_spk0_verify);
  if(hash_update(hc, msg, size) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_verify);

  /* Push the y values -- manually */
  by = NULL;
  if(pbcext_element_G1_to_bytes(&by, &len, y[0]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_verify);
  if(hash_update(hc, by, len) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_verify);
  mem_free(by); by = NULL;
  ...
  if(pbcext_element_G1_to_bytes(&bg, &len, g[0]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_verify);
  if(hash_update(hc, bg, len) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_verify);
  mem_free(bg); bg = NULL;
  ...
  for(j=0; j<ni; j++) {
    ...
    bi[2] = i[j][1] & 0xFF;
    bi[3] = (i[j][1] & 0xFF00) >> 8;
    if(hash_update(hc, bi, 4) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_verify);
  }
  ...
  if(pbcext_element_G1_to_bytes(&bprod, &len, prod[0]) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk0_verify);
  if(hash_update(hc, bprod, len) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_verify);
  mem_free(bprod); bprod = NULL;
  ...
  if(hash_finalize(hc) == IERROR) GOTOENDRC(IERROR, klapseq_spk0_verify);
...
klapseq_spk1_t* klapseq_spk1_init() {

  klapseq_spk1_t *pi;

  if (!(pi = mem_malloc(sizeof(klapseq_spk1_t))))
    return NULL;
...
int klapseq_spk1_free(klapseq_spk1_t *pi) {
...
int klapseq_spk1_get_size(klapseq_spk1_t *pi) {

  uint64_t size64, ss, sc, stau;
  
  if (!pi) {
    LOG_EINVAL(&logger, __FILE__, "klapseq_spk1_sign", __LINE__, LOGERROR);    
    return -1;
  }
...
int klapseq_spk1_export(byte_t **bytes,
		       uint64_t *len,
		       klapseq_spk1_t *pi) {
  ...
  if (!bytes || !len || !pi) {
    LOG_EINVAL(&logger, __FILE__, "klapseq_spk1_export", __LINE__, LOGERROR);
    return IERROR;
  }
  ...
  if(pbcext_dump_element_Fr_bytes(&bc, &clen, pi->c) == IERROR)
    GOTOENDRC(IERROR, klapseq_spk1_export);
...
klapseq_spk1_t* klapseq_spk1_import(byte_t *bytes, uint64_t *len) {
  ...
  if(!bytes || !len) {
    LOG_EINVAL(&logger, __FILE__, "klapseq_spk1_import", __LINE__,
	       LOGERROR);
    return NULL;
  }

  if(!(pi = klapseq_spk1_init())) {
    return NULL;
  }
  ...
  /* Get c */
  if(!(pi->c = pbcext_element_Fr_init()))
    GOTOENDRC(IERROR, klapseq_spk1_import);
 ...
 klapseq_spk1_import_end:

  if(rc == IERROR && pi) { klapseq_spk1_free(pi); pi = NULL; }
  
  return pi;  
  
}

int klapseq_spk1_sign(klapseq_spk1_t *pi,
  ...
  if (!(rr = pbcext_element_G2_init()))
    GOTOENDRC(IERROR, klapseq_spk1_sign);
...
int klapseq_spk1_verify(uint8_t *ok,
		       klapseq_spk1_t *pi,
		       pbcext_element_G1_t *g1,
		       pbcext_element_G1_t *g2,
		       pbcext_element_GT_t *e1,
  ...
  if (!pi || !g1 || !g2 || !e1 || !e2 || !msg || !size) {
    LOG_EINVAL(&logger, __FILE__, "klapseq_spk1_verify", __LINE__, LOGERROR);
    return IERROR;
  }
  ...
  if (!(aux = pbcext_element_GT_init()))
    GOTOENDRC(IERROR, klapseq_spk1_verify);
