...
#ifndef _KLAPSEQ_GML_H
#define _KLAPSEQ_GML_H
...
/** 
 * @struct klapseq_gml_entry_data_t
 * @brief Data stored in KLAPSEQ GML entries.
 */
typedef struct {
...
} klapseq_gml_entry_data_t;

/**
 * @fn gml_entry_t* klapseq_gml_entry_init()
...
/**
 * @fn int klapseq_gml_entry_free(gml_entry_t *entry)
...
/**
 * @fn int klapseq_gml_entry_get_size(gml_entry_t *entry)
...
/**
 * @fn int klapseq_gml_entry_export(byte_t **bytes,
 *                                uint32_t *size,
 *                                gml_entry_t *entry)
...
/**
 * @fn gml_t* klapseq_gml_entry_import(byte_t *bytes, uint32_t size)
...
/** 
 * @fn char* klapseq_gml_entry_to_string(klapseq_gml_entry_t *entry)
 * @brief Converts the received KLAPSEQ GML entry to a printable string.
 *
...
/** 
 * @fn int klapseq_gml_free(gml_t *gml)
...
/** 
 * @fn int klapseq_gml_insert(gml_t *gml, gml_entry_t *entry)
...
/** 
 * @fn int klapseq_gml_remove(gml_t *gml, uint64_t index)
...
/** 
 * @fn gml_entry_t* klapseq_gml_get(gml_t *gml, uint64_t index)
...
/**
 * @fn int klapseq_gml_export(byte_t **bytes, uint32_t *size, gml_t *gml)
...
/**
 * @fn gml_t* klapseq_gml_import(byte_t *bytes, uint32_t size);
...
/**
 * @var klapseq_gml_handle
 * @brief Set of functions for managing KLAPSEQ GMLs.
 */
static const gml_handle_t klapseq_gml_handle = {
  .scheme = GROUPSIG_KLAPSEQ_CODE, /**< Scheme code. */
  .init = &klapseq_gml_init, /**< GML initialization. */
  .free = &klapseq_gml_free, /**< GML free. */
  .insert = &klapseq_gml_insert, /**< Insert a new entry. */
  .remove = &klapseq_gml_remove, /**< Remove an existing entry. */
  .get = &klapseq_gml_get, /**< Gets (without removing) a specific entry. */
  .gimport = &klapseq_gml_import, /**< Import a GML at an external source. */
  .gexport = &klapseq_gml_export, /**< Export the GML to an external destination. */
  .entry_init = &klapseq_gml_entry_init, /**< Initializes a GML entry. */
  .entry_free = &klapseq_gml_entry_free, /**< Frees a GML entry. */
  .entry_get_size = &klapseq_gml_entry_get_size,  /**< Returns the size in bytes 
                                                   of a GML entry. */
  .entry_export = &klapseq_gml_entry_export, /**< Exports a GML entry. */
  .entry_import = &klapseq_gml_entry_import, /**< Imports a GML entry. */
  .entry_to_string = &klapseq_gml_entry_to_string, /**< Returns a human readable
