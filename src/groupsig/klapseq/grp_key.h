...
#ifndef _KLAPSEQ_GRP_KEY_H
#define _KLAPSEQ_GRP_KEY_H
...
/**
 * @struct klapseq_grp_key_t
 * @brief Structure for KLAPSEQ group keys.
 *
 * KLAPSEQ group keys. 
 */
typedef struct {
...
/**
 * @def KLAPSEQ_GRP_KEY_BEGIN_MSG
 * @brief Begin string to prepend to headers of files containing KLAPSEQ group keys
 */
#define KLAPSEQ_GRP_KEY_BEGIN_MSG "BEGIN KLAPSEQ GROUPKEY"

/**
 * @def KLAPSEQ_GRP_KEY_END_MSG
 * @brief End string to prepend to headers of files containing KLAPSEQ group keys
 */
#define KLAPSEQ_GRP_KEY_END_MSG "END KLAPSEQ GROUPKEY"

/** 
 * @fn groupsig_key_t* klapseq_grp_key_init()
...
/** 
 * @fn int klapseq_grp_key_free(groupsig_key_t *key)
...
/** 
 * @fn int klapseq_grp_key_copy(groupsig_key_t *dst, groupsig_key_t *src)
...
/** 
 * @fn int klapseq_grp_key_get_size(groupsig_key_t *key)
...
/**
 * @fn int klapseq_grp_key_export(byte_t **bytes, uint32_t *size, groupsig_key_t *key)
 * @brief Writes a bytearray representation of the given key, with format:
 *
 *  | KLAPSEQ_CODE | KEYTYPE | size_g | g | size_gg | gg | size_XX | XX | 
 ...
 * @return IOK or IERROR
 */
int klapseq_grp_key_export(byte_t **bytes, uint32_t *size, groupsig_key_t *key);

/** 
 * @fn groupsig_key_t* klapseq_grp_key_import(byte_t *source, uint32_t size)
 * @brief Imports a group key.
 *
 * Imports a KLAPSEQ group key from the specified array of bytes.
 * 
...
/** 
 * @fn char* klapseq_grp_key_to_string(groupsig_key_t *key)
...
/**
 * @var klapseq_grp_key_handle
 * @brief The set of functions to manage KLAPSEQ group keys.
 */
static const grp_key_handle_t klapseq_grp_key_handle = {
  .code = GROUPSIG_KLAPSEQ_CODE, /**< Scheme. */
  .init = &klapseq_grp_key_init, /**< Initialize group keys. */
  .free = &klapseq_grp_key_free, /**< Free group keys. */
  .copy = &klapseq_grp_key_copy, /**< Copy group keys. */
  .gexport = &klapseq_grp_key_export, /**< Export group keys. */
  .gimport = &klapseq_grp_key_import, /**< Import group keys. */
  .to_string = &klapseq_grp_key_to_string, /**< Convert to printable strings. */
  .get_size = &klapseq_grp_key_get_size,
};
