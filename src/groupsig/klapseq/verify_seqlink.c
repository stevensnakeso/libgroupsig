...
static int _klapseq_verify_sequence2(uint8_t *ok,
				    klapseq_proof_t *proof,
				    groupsig_signature_t **sigs,
            int header) {
  ...
  for (i=1; i<n; i++) {
    ...
    {
      if(memcmp(hc->hash, klapseq_sig->seq->seq3, klapseq_sig->seq->len3)) {
        *ok = 0;
        GOTOENDRC(IOK, _klapseq_verify_sequence);
      }
    }
...
int klapseq_verify_seqlink2(uint8_t *ok,
			   groupsig_key_t *grpkey,
			   groupsig_proof_t *proof,
			   message_t *msg,
			   groupsig_signature_t **sigs,
  ...
  /* Recompute the sequence information from the proof, and check it */
  if (_klapseq_verify_sequence2(&_ok, proof->proof, sigs, header) == IERROR)
      GOTOENDRC(IERROR, klapseq_verify_link);
