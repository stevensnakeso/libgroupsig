...
int bap24_proof_free(groupsig_proof_t *proof) {
  ...
  if (!proof) {
    LOG_EINVAL_MSG(&logger, __FILE__, "bap24_proof_free", __LINE__,
...
int bap24_proof_get_size(groupsig_proof_t *proof) {
  ...
  if(!proof || proof->scheme != GROUPSIG_BAP24_CODE) {
    LOG_EINVAL(&logger, __FILE__, "bap24_proof_get_size", __LINE__, LOGERROR);
    return -1;
  }
...
int bap24_proof_export(byte_t **bytes, uint32_t *size, groupsig_proof_t *proof) {
  ...
  if(!proof || proof->scheme != GROUPSIG_BAP24_CODE) {
    LOG_EINVAL(&logger, __FILE__, "bap24_proof_export", __LINE__, LOGERROR);
    return IERROR;
  }
  ...
  if ((_size = bap24_proof_get_size(proof)) == -1) {
    return IERROR;
  }
  ...
  if (_size != proof_len+1) {
    LOG_ERRORCODE_MSG(&logger, __FILE__, "bap24_proof_export", __LINE__,
...
groupsig_proof_t* bap24_proof_import(byte_t *source, uint32_t size) {
  ...
  if(!source || !size) {
    LOG_EINVAL(&logger, __FILE__, "bap24_proof_import", __LINE__, LOGERROR);
    return NULL;
  }

  rc = IOK;

  if(!(proof = bap24_proof_init())) {
    return NULL;
  }
  ...
  if (scheme != proof->scheme) {
    LOG_ERRORCODE_MSG(&logger, __FILE__, "bap24_proof_import", __LINE__, 
 ...
 bap24_proof_import_end:

  if(rc == IERROR && proof) { bap24_proof_free(proof); proof = NULL; }
...
char* bap24_proof_to_string(groupsig_proof_t *proof) {
  ...
  if(!proof || proof->scheme != GROUPSIG_BAP24_CODE) {
    LOG_EINVAL(&logger, __FILE__, "bap24_proof_to_string", __LINE__, LOGERROR);
    return NULL;
  }
