...
#ifndef _BAP24_GRP_KEY_H
#define _BAP24_GRP_KEY_H
...
/**
 * @struct bap24_grp_key_t
 * @brief Structure for BAP24 group keys.
 *
 * BAP24 group keys. 
 */
typedef struct {
...
/**
 * @def BAP24_GRP_KEY_BEGIN_MSG
 * @brief Begin string to prepend to headers of files containing BAP24 group keys
 */
#define BAP24_GRP_KEY_BEGIN_MSG "BEGIN BAP24 GROUPKEY"

/**
 * @def BAP24_GRP_KEY_END_MSG
 * @brief End string to prepend to headers of files containing BAP24 group keys
 */
#define BAP24_GRP_KEY_END_MSG "END BAP24 GROUPKEY"

/** 
 * @fn groupsig_key_t* bap24_grp_key_init()
...
/** 
 * @fn int bap24_grp_key_free(groupsig_key_t *key)
...
/** 
 * @fn int bap24_grp_key_copy(groupsig_key_t *dst, groupsig_key_t *src)
...
/** 
 * @fn int bap24_grp_key_get_size(groupsig_key_t *key)
...
/**
 * @fn int bap24_grp_key_export(byte_t **bytes, uint32_t *size, groupsig_key_t *key)
 * @brief Writes a bytearray representation of the given key, with format:
 *
 *  | BAP24_CODE | KEYTYPE | size_g | g | size_gg | gg | size_X | X | size_Y | Y |
 *
 ...
 * @return IOK or IERROR
 */
int bap24_grp_key_export(byte_t **bytes, uint32_t *size, groupsig_key_t *key);

/** 
 * @fn groupsig_key_t* bap24_grp_key_import(byte_t *source, uint32_t size)
 * @brief Imports a group key.
 *
 * Imports a BAP24 group key from the specified array of bytes.
 * 
...
/** 
 * @fn char* bap24_grp_key_to_string(groupsig_key_t *key)
...
/**
 * @var bap24_grp_key_handle
 * @brief The set of functions to manage BAP24 group keys.
 */
static const grp_key_handle_t bap24_grp_key_handle = {
  .code = GROUPSIG_BAP24_CODE, /**< Scheme. */
  .init = &bap24_grp_key_init, /**< Initialize group keys. */
  .free = &bap24_grp_key_free, /**< Free group keys. */
  .copy = &bap24_grp_key_copy, /**< Copy group keys. */
  .gexport = &bap24_grp_key_export, /**< Export group keys. */
  .gimport = &bap24_grp_key_import, /**< Import group keys. */
  .to_string = &bap24_grp_key_to_string, /**< Convert to printable strings. */
  .get_size = &bap24_grp_key_get_size,
};
