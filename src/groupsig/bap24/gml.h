...
/** 
 * @struct bap24_gml_entry_data_t
 * @brief Data stored in BAP24 GML entries.
 */
typedef struct {
...
/**
 * @fn gml_entry_t* bap24_gml_entry_init()
...
/**
 * @fn int bap24_gml_entry_free(gml_entry_t *entry)
...
/**
 * @fn int bap24_gml_entry_get_size(gml_entry_t *entry)
...
/**
 * @fn int bap24_gml_entry_export(byte_t **bytes,
 *                                uint32_t *size,
 *                                gml_entry_t *entry)
...
/**
 * @fn gml_t* bap24_gml_entry_import(byte_t *bytes, uint32_t size)
...
/** 
 * @fn char* bap24_gml_entry_to_string(bap24_gml_entry_t *entry)
 * @brief Converts the received BAP24 GML entry to a printable string.
 *
...
/** 
 * @fn int bap24_gml_free(gml_t *gml)
...
/** 
 * @fn int bap24_gml_insert(gml_t *gml, gml_entry_t *entry)
...
/** 
 * @fn int bap24_gml_remove(gml_t *gml, uint64_t index)
...
/** 
 * @fn gml_entry_t* bap24_gml_get(gml_t *gml, uint64_t index)
...
/**
 * @fn int bap24_gml_export(byte_t **bytes, uint32_t *size, gml_t *gml)
...
/**
 * @fn gml_t* bap24_gml_import(byte_t *bytes, uint32_t size);
...
/**
 * @var bap24_gml_handle
 * @brief Set of functions for managing BAP24 GMLs.
 */
static const gml_handle_t bap24_gml_handle = {
  .scheme = GROUPSIG_BAP24_CODE, /**< Scheme code. */
  .init = &bap24_gml_init, /**< GML initialization. */
  .free = &bap24_gml_free, /**< GML free. */
  .insert = &bap24_gml_insert, /**< Insert a new entry. */
  .remove = &bap24_gml_remove, /**< Remove an existing entry. */
  .get = &bap24_gml_get, /**< Gets (without removing) a specific entry. */
  .gimport = &bap24_gml_import, /**< Import a GML at an external source. */
  .gexport = &bap24_gml_export, /**< Export the GML to an external destination. */
  .entry_init = &bap24_gml_entry_init, /**< Initializes a GML entry. */
  .entry_free = &bap24_gml_entry_free, /**< Frees a GML entry. */
  .entry_get_size = &bap24_gml_entry_get_size,  /**< Returns the size in bytes 
                                                   of a GML entry. */
  .entry_export = &bap24_gml_entry_export, /**< Exports a GML entry. */
  .entry_import = &bap24_gml_entry_import, /**< Imports a GML entry. */
  .entry_to_string = &bap24_gml_entry_to_string, /**< Returns a human readable
