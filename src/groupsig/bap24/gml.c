...
gml_t* bap24_gml_init() {

  gml_t *gml;

  if(!(gml = (gml_t *) malloc(sizeof(gml_t)))) {
    LOG_ERRORCODE(&logger, __FILE__, "bap24_gml_init", __LINE__, errno, LOGERROR);
    return NULL;
  }

  gml->scheme = GROUPSIG_BAP24_CODE;
  gml->entries = NULL;
  gml->n = 0;
...
int bap24_gml_free(gml_t *gml) {

  uint64_t i;

  if(!gml || gml->scheme != GROUPSIG_BAP24_CODE) {
    LOG_EINVAL_MSG(&logger, __FILE__, "bap24_gml_free", __LINE__,
  ...
  if (gml->entries) {
    for(i=0; i<gml->n; i++) {
      bap24_gml_entry_free(gml->entries[i]); gml->entries[i] = NULL;
    }
...
int bap24_gml_insert(gml_t *gml, gml_entry_t *entry) {

  if(!gml || gml->scheme != GROUPSIG_BAP24_CODE ||
     gml->scheme != entry->scheme) {
    LOG_EINVAL(&logger, __FILE__, "bap24_gml_insert", __LINE__, LOGERROR);
    return IERROR;
  }

  if(!(gml->entries = (gml_entry_t **)
       realloc(gml->entries, sizeof(gml_entry_t *)*(gml->n+1)))) {
    LOG_ERRORCODE(&logger, __FILE__, "bap24_gml_insert", __LINE__, errno,
		  LOGERROR);
    return IERROR;
  }
...
int bap24_gml_remove(gml_t *gml, uint64_t index) {

  if(!gml || gml->scheme != GROUPSIG_BAP24_CODE) {
    LOG_EINVAL(&logger, __FILE__, "bap24_gml_remove", __LINE__, LOGERROR);
    return IERROR;
  }

  if(index >= gml->n) {
    LOG_EINVAL_MSG(&logger, __FILE__, "bap24_gml_remove", __LINE__,
...
gml_entry_t* bap24_gml_get(gml_t *gml, uint64_t index) {

  if(!gml || gml->scheme != GROUPSIG_BAP24_CODE) {
    LOG_EINVAL(&logger, __FILE__, "bap24_gml_get", __LINE__, LOGERROR);
    return NULL;
  }

  if(index >= gml->n) {
    LOG_EINVAL_MSG(&logger, __FILE__, "bap24_gml_get", __LINE__, "Invalid index.",
  		   LOGERROR);
    return NULL;
  }
...
int bap24_gml_export(byte_t **bytes, uint32_t *size, gml_t *gml) {
  ...
  if (!bytes || !size || !gml || gml->scheme != GROUPSIG_BAP24_CODE) {
    LOG_EINVAL(&logger, __FILE__, "bap24_gml_export", __LINE__, LOGERROR);
    return IERROR;
  }
  ...
  if (!(_bytes = mem_malloc(sizeof(uint64_t))))
    GOTOENDRC(IERROR, bap24_gml_export);
...
gml_t* bap24_gml_import(byte_t *bytes, uint32_t size) {
  ...
  if(!bytes || !size) {
    LOG_EINVAL(&logger, __FILE__, "bap24_gml_import", __LINE__, LOGERROR);
    return NULL;
  }
  ...
  if (!(gml = bap24_gml_init())) GOTOENDRC(IERROR, bap24_gml_import);
  ...
  if (!(gml->entries = mem_malloc(sizeof(gml_entry_t *)*gml->n)))
    GOTOENDRC(IERROR, bap24_gml_import);

  /* Import the entries one by one */
  for (i=0; i<gml->n; i++) {

    if (!(gml->entries[i] = bap24_gml_entry_import(&bytes[read], _entry_size)))
      GOTOENDRC(IERROR, bap24_gml_import);

    if ((entry_size = bap24_gml_entry_get_size(gml->entries[i])) == -1)
      GOTOENDRC(IERROR, bap24_gml_import);
...
gml_entry_t* bap24_gml_entry_init() {

  gml_entry_t *entry;

  if(!(entry = (gml_entry_t *) malloc(sizeof(gml_entry_t)))) {
    LOG_ERRORCODE(&logger, __FILE__, "bap24_gml_entry_init", __LINE__,
		  errno, LOGERROR);
    return NULL;
  }

  entry->scheme = GROUPSIG_BAP24_CODE;
  entry->id = UINT64_MAX;
  entry->data = NULL;
...
int bap24_gml_entry_free(gml_entry_t *entry) {

  bap24_gml_entry_data_t *data;
  int rc;

  if(!entry) {
    LOG_EINVAL_MSG(&logger, __FILE__, "bap24_gml_entry_free", __LINE__,
		   "Nothing to free.", LOGWARN);
    return IOK;
  }

  rc = IOK;
  data = (bap24_gml_entry_data_t *) entry->data;
...
int bap24_gml_entry_export(byte_t **bytes,
			  uint32_t *size,
			  gml_entry_t *entry) {
  ...
  if (!bytes || !size || !entry) {
    LOG_EINVAL(&logger, __FILE__, "bap24_gml_entry_export", __LINE__, LOGERROR);
    return IERROR;
  }

  /* Calculate size */
  if ((_size = bap24_gml_entry_get_size(entry)) == -1) return IERROR;
  ...
  /* Next, dump the data, which for BAP24 is tau (G1 element) and
     ttau (G2 element) */
  __bytes = &_bytes[offset];
  if (pbcext_dump_element_G1_bytes(&__bytes,
				   &len,
				   ((bap24_gml_entry_data_t *) entry->data)->tau) == IERROR) {
    mem_free(_bytes); _bytes = NULL;
    return IERROR;
  }
  offset += len;
  ...
  /* Sanity check */
  if (offset != _size) {
    LOG_ERRORCODE_MSG(&logger, __FILE__, "bap24_gml_entry_export", __LINE__,
...
gml_entry_t* bap24_gml_entry_import(byte_t *bytes, uint32_t size) {
  ...
  if (!bytes || !size) {
    LOG_EINVAL(&logger, __FILE__, "bap24_gml_entry_import", __LINE__, LOGERROR);
    return NULL;
  }

  if (!(entry = bap24_gml_entry_init())) return NULL;
  ...
  if (!(entry->data = mem_malloc(sizeof(bap24_gml_entry_data_t)))) {
    bap24_gml_entry_free(entry); entry = NULL;
    return NULL;
  }

  if(!(((bap24_gml_entry_data_t *)entry->data)->tau = pbcext_element_G1_init())) {
    bap24_gml_entry_free(entry); entry = NULL;
    return NULL;
  }

  if (pbcext_get_element_G1_bytes(((bap24_gml_entry_data_t *)entry->data)->tau,
				  &len,
				  &bytes[offset]) == IERROR) {
    bap24_gml_entry_free(entry); entry = NULL;
    return NULL;
  ...
  if(!(((bap24_gml_entry_data_t *)entry->data)->ttau = pbcext_element_G2_init())) {
    bap24_gml_entry_free(entry); entry = NULL;
    return NULL;
  }

  if (pbcext_get_element_G2_bytes(((bap24_gml_entry_data_t *)entry->data)->ttau,
				  &len,
				  &bytes[offset]) == IERROR) {
    bap24_gml_entry_free(entry); entry = NULL;
    return NULL;
...
char* bap24_gml_entry_to_string(gml_entry_t *entry) {
  ...
  if(!(sentry = (char *) mem_malloc(sizeof(char)*sentry_len+1))) {
    LOG_ERRORCODE(&logger, __FILE__, "bap24_gml_entry_to_string", __LINE__, errno,
		  LOGERROR);
    free(stau); stau = NULL;
