...
int bap24_signature_free(groupsig_signature_t *sig) {
  ...
  if(sig->sig) {
    ...
    if(bap24_sig->cnym3){
      pbcext_element_G2_free(bap24_sig->cnym3);
      bap24_sig->cnym3 = NULL;
    }
    // if(bap24_sig->D1){
...
int bap24_signature_copy(groupsig_signature_t *dst, groupsig_signature_t *src) {
  ...
  if(pbcext_element_GT_set(bap24_dst->cnym2, bap24_src->cnym2) == IERROR)
    GOTOENDRC(IERROR, bap24_signature_copy);
  if(!(bap24_dst->cnym3 = pbcext_element_G2_init()))
    GOTOENDRC(IERROR, bap24_signature_copy);
  if(pbcext_element_G2_set(bap24_dst->cnym3, bap24_src->cnym3) == IERROR)
    GOTOENDRC(IERROR, bap24_signature_copy);
 ...
 bap24_signature_copy_end:
  ...
  if(rc == IERROR) {
    ...
    if(bap24_dst->cnym3){
      pbcext_element_G2_free(bap24_dst->cnym3);
      bap24_dst->cnym3 = NULL;
    }
    // if (bap24_dst->D1){
...
int bap24_signature_get_size(groupsig_signature_t *sig) {
  ...
  bap24_signature_t *bap24_sig;
  //uint64_t size64, ssigma1, ssigma2, sc,  sz_zeta1, sz_zeta2, sz_theta1, sz_theta2, sz_sk, sz_uid, sz_w, sz_alpha, sB1, sB2, shscp, scnym1, scnym2,scnym3, sD1, sD2, sD3, sD4, sD5, sD6;
  uint64_t size64, ssigma1, ssigma2, sc,  sz_zeta1, sz_zeta2, sz_theta1, sz_theta2, sz_sk, sz_uid, sz_w, sz_alpha, sB1, sB2, shscp, scnym1, scnym2,scnym3;
  if(!sig || sig->scheme != GROUPSIG_BAP24_CODE) {
  ...
  if(pbcext_element_G2_byte_size(&scnym3) == IERROR) return -1;
 ...
 // size64 = sizeof(uint8_t) + sizeof(int)*23 + ssigma1 + ssigma2 +  sc  + sz_zeta1 + sz_zeta2 + sz_theta1 + sz_theta2 + sz_sk + sz_uid + sz_w + sz_alpha + sB1 + sB2 + shscp + scnym1 + scnym2 + scnym3 + sD1 + sD2 + sD3 + sD4 + sD5 + sD6;
  size64 = sizeof(uint8_t) + sizeof(int)*17 + ssigma1 + ssigma2 +  sc  + sz_zeta1 + sz_zeta2 + sz_theta1 + sz_theta2 + sz_sk + sz_uid + sz_w + sz_alpha + sB1 + sB2 + shscp + scnym1 + scnym2 + scnym3;
...
int bap24_signature_export(byte_t **bytes,
  ...
  /* Dump cnym3 */
  __bytes = &_bytes[ctr];
  if(pbcext_dump_element_G2_bytes(&__bytes, &len, bap24_sig->cnym3) == IERROR) 
    GOTOENDRC(IERROR, bap24_signature_export);
  ctr += len;
...
groupsig_signature_t* bap24_signature_import(byte_t *source, uint32_t size) {
  ...
  /* Get cnym3 */
  if(!(bap24_sig->cnym3 = pbcext_element_G2_init()))
    GOTOENDRC(IERROR, bap24_signature_import);
  if(pbcext_get_element_G2_bytes(bap24_sig->cnym3, &len, &source[ctr]) == IERROR)
    GOTOENDRC(IERROR, bap24_signature_import);
  ctr += len;
