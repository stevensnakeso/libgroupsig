...
namespace groupsig {
  ...
  // The fixture for testing BAP24 scheme.
  class BAP24Test : public ::testing::Test {
  protected:
    ...
    BAP24Test() {
      ...
      rc = groupsig_init(GROUPSIG_BAP24_CODE, time(NULL));
      EXPECT_EQ(rc, IOK);

      mgrkey = groupsig_mgr_key_init(GROUPSIG_BAP24_CODE);
      EXPECT_NE(mgrkey, nullptr);

      grpkey = groupsig_grp_key_init(GROUPSIG_BAP24_CODE);
      EXPECT_NE(grpkey, nullptr);

      gml = gml_init(GROUPSIG_BAP24_CODE);
	...
	free(memkey); memkey = NULL;
      ...
      groupsig_clear(GROUPSIG_BAP24_CODE);
    }
  ...
  TEST_F(BAP24Test, GetCodeFromStr) {

    int rc;
    uint8_t scheme;

    rc = groupsig_get_code_from_str(&scheme, (char *) GROUPSIG_BAP24_NAME);
    EXPECT_EQ(rc, IOK);

    EXPECT_EQ(scheme, GROUPSIG_BAP24_CODE);

  }

  // Tests that the BAP24 constructor creates the required keys.
  TEST_F(BAP24Test, CreatesGrpAndMgrKeys) {

    /* Scheme is set to BAP24 */
    EXPECT_EQ(grpkey->scheme, GROUPSIG_BAP24_CODE);
    EXPECT_EQ(mgrkey->scheme, GROUPSIG_BAP24_CODE);

  }

  /* groupsig_get_joinstart must return 0 */
  TEST_F(BAP24Test, CheckJoinStart) {

    int rc;
    uint8_t start;

    rc = groupsig_get_joinstart(GROUPSIG_BAP24_CODE, &start);
    EXPECT_EQ(rc, IOK);
  ...
  TEST_F(BAP24Test, CheckJoinSeq) {

    int rc;
    uint8_t seq;

    rc = groupsig_get_joinseq(GROUPSIG_BAP24_CODE, &seq);
    EXPECT_EQ(rc, IOK);
  ...
  TEST_F(BAP24Test, InitializeSignature) {
    ...
    EXPECT_EQ(sig->scheme, GROUPSIG_BAP24_CODE);
  ...
  TEST_F(BAP24Test, SignVerifyValid) {
  ...
  TEST_F(BAP24Test, SignVerifyWrongMessage) {
  ...
  TEST_F(BAP24Test, OpenSignatureValidProof) {
    ...
    /* Open */
    proof = groupsig_proof_init(GROUPSIG_BAP24_CODE);
  ...
  TEST_F(BAP24Test, OpenSignatureWrongProof) {
  ...
  TEST_F(BAP24Test, GrpKeyExportImport) {
    ...
    dst = groupsig_grp_key_import(GROUPSIG_BAP24_CODE, bytes, size);
  ...
  TEST_F(BAP24Test, GrpKeyCopy) {
    ...
    dst = groupsig_grp_key_init(GROUPSIG_BAP24_CODE);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(BAP24Test, MgrKeyExportImport) {
    ...
    /* Import the group key */
    dst = groupsig_mgr_key_import(GROUPSIG_BAP24_CODE, bytes, size);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(BAP24Test, MgrKeyCopy) {
    ...
    dst = groupsig_mgr_key_init(GROUPSIG_BAP24_CODE);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(BAP24Test, MemKeyExportImport) {
    ...
    /* Import the group key */
    dst = groupsig_mem_key_import(GROUPSIG_BAP24_CODE, bytes, size);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(BAP24Test, MemKeyCopy) {
   ...
   /* Add one member */
    ...
    dst = groupsig_mem_key_init(GROUPSIG_BAP24_CODE);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(BAP24Test, SignatureToString) {
  ...
  TEST_F(BAP24Test, SignatureCopy) {
  ...
  TEST_F(BAP24Test, SignatureExportImport) {
  ...
  TEST_F(BAP24Test, GmlExportImport) {
    ...
    /* Import */
    imported = gml_import(GROUPSIG_BAP24_CODE, bytes, size);
    EXPECT_NE(imported, nullptr);
  ...
  TEST_F(BAP24Test, OpenSignatureValidProofAfterGMLExportImport) {
