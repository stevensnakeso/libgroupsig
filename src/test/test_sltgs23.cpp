...
namespace groupsig {
  ...
  class SLTGS23Test : public ::testing::Test {
  protected:
  ...
  TEST_F(SLTGS23Test, GetCodeFromStr) {

    int rc;
    uint8_t scheme;

    rc = groupsig_get_code_from_str(&scheme, (char *) GROUPSIG_SLTGS23_NAME);
    EXPECT_EQ(rc, IOK);

    EXPECT_EQ(scheme, GROUPSIG_SLTGS23_CODE);

  }

  // Tests that the SLTGS23 constructor creates the required keys.
  TEST_F(SLTGS23Test, CreatesGrpAndMgrKeys) {
  ...
  TEST_F(SLTGS23Test, CheckJoinStart) {
  ...
  TEST_F(SLTGS23Test, CheckJoinSeq) {
  ...
  TEST_F(SLTGS23Test, InitializeSignature) {
  ...
  TEST_F(SLTGS23Test, SignVerifyValid) {
  ...
  TEST_F(SLTGS23Test, SignVerifyWrongMessage) {
...
/* Successfull blind-convert-unblind of 1 signature */
  ...
  TEST_F(SLTGS23Test, GrpKeyExportImport) {
    ...
    /* Import the group key */
    dst = groupsig_grp_key_import(GROUPSIG_SLTGS23_CODE, bytes, size);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(SLTGS23Test, MemKeyExportImport) {
    ...
    /* Import the group key */
    dst = groupsig_mem_key_import(GROUPSIG_SLTGS23_CODE, bytes, size);
    EXPECT_NE(dst, nullptr);
...
/** Blind key tests **/
  ...
  TEST_F(SLTGS23Test, SignatureToString) {
  ...
  TEST_F(SLTGS23Test, SignatureCopy) {
